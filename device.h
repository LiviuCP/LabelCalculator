...
public:
    ...
    */
    void parseInputData(const string &s, int &pos, bool &error, ofstream &err);

    // appends the description and label to the string
    void writeDescriptionAndLabel(string& out) const;
...
protected:
    void handleError(ofstream& err); // error handling function, will write any error to output file by using the err stream

    string mDeviceName;  // device name (e.g. for PDU-A the name is "A")
    bool mIsFirstDevice; // true if first of the two devices in each connection entered in the input .csv file
    string mDeviceType;  // device type (ex: "pdu" for basic PDUs without load segments)
    string mDescription; // text to be written in the source (first device) / destination (second device) field of the labelling table (device description)
    string mLabel;       // text to be written in the source (first device) / destination (second device) field of the labelling table (device label)
    ...
    vector <string*> mInputData; // reference to substrings storing the fields parsed by parseInputData()
